...
def process_file(directory_path, output_directory, filename):
    try:
        ...
        if filename.endswith('.pdf'):
            ...
            first_page = pages[0]
            last_page = pages[-1 if num_pages > 1 else 0]  # Handle case with a single page

            # Extract the text from the first and last page images
            first_page_text = pytesseract.image_to_string(first_page).strip()
            last_page_text = pytesseract.image_to_string(last_page).strip()
